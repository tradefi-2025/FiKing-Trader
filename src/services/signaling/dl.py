...
class SignalingDataLoader:
    ...
    def fetch_news_embeddings_dataset(self,equity: str, prompt: str = "", resources: list = None) -> pd.DataFrame:
        '''
        ...
        '''
        # Placeholder implementation — replace with actual news fetching and embedding logic
        news = self.db.get_news_embeddings(equity=self.equity, start=None, end=None)
    ...
    def _align_news_to_windows(self, end_timestamps: list) -> torch.Tensor:
        ...
        Returns:
            List[torch.Tensor] of length M, each tensor shape (N_i, embedding_dim)
            where N_i is the number of articles for that window.
        """
        ...
        timestamps = news['timestamps']  # List[datetime]
        embeddings = news['embeddings']  # (N, embedding_dim)
        
        if embeddings.numel() == 0:
            # No news at all — return zero vectors for each window
        ...
        return all_window_embeddings  # List of (N_i, embedding_dim) tensors
    
    def test_align_news_to_windows(self):
        """
        Test that news embeddings are correctly aligned to OHLCV windows.
        """
        # Create dummy news data with timestamps and embeddings
        now = pd.Timestamp.now()
        end_timestamps = [now - pd.Timedelta(days=10), now - pd.Timedelta(days=5), now]
        news_data= self._align_news_to_windows(end_timestamps)
        for end_ts, window_embs in zip(end_timestamps, news_data):
...
if __name__ == "__main__":
    dl= SignalingDataLoader(metadata={
        "equity": "AAPL",
        "time_frequency": "1h",
        "observation_horizon": 50,
        "prediction_horizon": 1,
        "news_observation_horizon": 50,
        "news_retrieval_prompt": "Fetch news relevant to AAPL stock price movements.",
        "news_resources": ["Reuters", "Bloomberg"],
    })
    dl.test_align_news_to_windows()